...
> 最終更新: 2026-04-05 / app.js 1,917行 / v6（頭痛トラッキング追加済み）
...
| サブスコア | 満点 | ロジック | 設計根拠 |
|-----------|------|---------|---------|
| 睡眠時間 | 8 | 8h=8, 7.5h=7, 7h=6, 6.5h=5, 6h=4, 5.5h=2, 5h=1, 4.5h-=0 | SAS患者は睡眠時間が質に直結 |
| 睡眠の質 | 6 | great=6, ok=4, light=2, terrible=0 | 主観的品質評価 |
| CPAP装着 | 6 | バッチリ=6, 途中外した=3, つけなかった=0 | SAS治療の要。当初2択→3段階に改善 |
| 痛みVAS | 13 | `13 × (1 - pain/100)` | ヘルニア患者の重要指標。痛みなし=13点 |
| 頭痛 | 2 | なし=2, 軽い=1, ひどい=0 | 体調シグナル。v6追加（元々painが15点→13+2に分割） |

### カテゴリ2: 生活習慣（25点満点）

| サブスコア | 満点 | ロジック |
|-----------|------|---------|
| 夕食タイプ | 8 | ゆかごはん=8, 外食=4, 外食&飲酒=1 |
| 夕食時間 | 5 | 19時前=5, 19-21時=3, 21時以降=1 |
| 健康習慣 | 12 | 5タグ × 2.4点（端数丸め） |

### カテゴリ3: パフォーマンス & SOS（20点満点）

| サブスコア | 満点 | ロジック |
|-----------|------|---------|
| パフォーマンスVAS | 10 | `10 × perfVas/100`（夜チェックアウトの値） |
| 疲労度VAS | 6 | `6 × (1 - fatigue/100)`（朝チェックインの値） |
| SOS勝利 | 4 | 1勝=2点, 最大4点（2勝まで） |

### カテゴリ4: リカバリー行動（20点満点）

| サブスコア | 満点 | ロジック |
|-----------|------|---------|
| 朝のDRAMMA振り返り | 8 | 6カテゴリ中の充足数 × (8/6)。全カテゴリ充足で8点 |
| 終業リチュアル | 6 | 5+実行=6, 3-4=4, 1-2=2, 0=0 |
| 休養プラン | 6 | 5+選択=6, 3-4=4, 1-2=2, 0=0 |
...
元々4つの進化パターン（`RESIST_未来の進化構想.md` 参照）が提示されていた。現在の実装状況：

| パターン | テーマ | 状況 |
|---------|--------|------|
| A: 睡眠・回復最適化 | 夜チェックアウト、CPAP追跡、夕食時間記録 | **ほぼ実装済み** |
| B: 身体機能・ペインマネジメント | 痛みVAS、体重-痛み相関の可視化 | **基盤実装済み**（重ねグラフは未実装） |
| C: 認知エネルギー・メンタル | バッテリー残量予測、カフェイン管理 | **未着手** |
| D: 家族共創PBL | スペシャルデイの具体的クエスト化、感謝ループ | **一部実装**（チケット制度のみ） |

### 今後の候補（優先度順の提案）

1. **体重-痛み重ねグラフ** — 「体重が減ると本当に痛みが和らぐ」を視覚的に証明（パターンBの完成）
2. **カフェイン管理** — 13:40デッドライン表示、離脱頭痛の防止（パターンC）
3. **認知バッテリー（MP）予測** — 睡眠時間+多忙度から今日の精神バッテリーを算出、早帰りアラート
4. **スペシャルデイ・クエスト化** — チケットを「家族への還元アクション」に交換する仮想ショップ
5. **ファイル分割** — app.jsが2000行規模になったらモジュール分割を検討（ESM or ビルドツール導入）
